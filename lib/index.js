 ...
 * while new models on an existing type (e.g. another OpenRouter model) can be
 * pushed purely via remote help.json. */
const REGISTRY = {
	zai: { key: "zai", apiKeyEnv: "ZAI_API_KEY" },
	openrouter: { key: "openrouter", apiKeyEnv: "OPENROUTER_API_KEY" }
};
 ...
 * 4-space indent are NOT consumed — this is what keeps zai & openrouter from
		...
		if (req.method === "POST" && req.url === "/configure-openrouter") {
			...
			req.on("end", () => {
				try {
					...
					// model list may be carried by the remote card; fall back to the built-in spec.
					const base = OPENROUTER_PROVIDER;
					const models = Array.isArray(parsed.models) && parsed.models.length ? parsed.models : base.models;
					writeCredential(trimmed, base.apiKeyEnv);
					upsertProviderBlock(base.key, renderProviderBlock({ ...base, models }));
					writeAgentDefault({ provider: base.key, model: models[0].id });
					res.writeHead(200, { "Content-Type": "application/json" });
					res.end(JSON.stringify({ ok: true, configured: true, defaultModel: { provider: base.key, model: models[0].id } }));
				} catch (e) {
					res.writeHead(400, { "Content-Type": "application/json" });
					res.end(JSON.stringify({ ok: false, error: String((e && e.message) || e) }));
				}
