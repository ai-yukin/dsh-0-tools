...
/** Write / replace a credential line in .credentials.yaml (default ZAI_API_KEY). */
function writeCredential(key, envName = "ZAI_API_KEY") {
	...
	for (let i = 0; i < lines.length; i++) {
		if (new RegExp("^\\s*" + envName + "\\s*:").test(lines[i])) {
			lines[i] = envName + ": " + key;
			replaced = true;
			break;
		}
...
/* OpenRouter (Ox-Alpha 匿名免费预览) — not in pi-ai's built-in catalog, so the
 * route spells out its own wire protocol (api/baseURL) and full model spec.
 * See dsh-llm-pi-ai: providers is a dict keyed by arbitrary route; openai-
 * completions + compat handle OpenRouter's reasoning normalization. */
const OPENROUTER_PROVIDER = {
	key: "openrouter",
	apiKeyEnv: "OPENROUTER_API_KEY",
	displayName: "OpenRouter（Ox-Alpha 免费预览）",
	api: "openai-completions",
	baseURL: "https://openrouter.ai/api/v1",
	models: [
		{
			id: "stealth/ox-alpha",
			name: "Ox Alpha（OpenRouter 免费预览）",
...
/** Whether the openrouter provider is present in settings.yaml. */
...
function upsertProviderBlock(providerKey, blockText) {
	...
	backupIfExists(p);
	let text = fs.existsSync(p) ? fs.readFileSync(p, "utf8") : "";
	// 1) drop any previous block for this provider key
	const blockRe = new RegExp("^    " + providerKey + ":[ \\t]*\\r?\\n(?:[ \\t].*\\r?\\n)*", "m");
...
export function apply(ctx) {
	...
	const handler = (req, res) => {
		...
		// One-click OpenRouter (Ox-Alpha) install: write credential + provider
		// route + switch the agent default model, mirroring the zai flow.
		if (req.method === "POST" && req.url === "/configure-openrouter") {
			let body = "";
			req.on("data", (c) => {
				body += c;
				if (body.length > 1e6) req.destroy();
			...
			req.on("end", () => {
				try {
					const parsed = JSON.parse(body || "{}");
					const key = parsed && parsed.key;
					if (!key || typeof key !== "string" || !key.trim()) throw new Error("缺少有效的 OpenRouter API Key");
					const trimmed = key.trim();
					if (!/^sk-or-/i.test(trimmed)) throw new Error("Key 看起来不是 OpenRouter 的（通常以 sk-or- 开头），请确认后重试");
					writeCredential(trimmed, OPENROUTER_PROVIDER.apiKeyEnv);
					upsertProviderBlock(OPENROUTER_PROVIDER.key, renderProviderBlock(OPENROUTER_PROVIDER));
					writeAgentDefault({ provider: "openrouter", model: "stealth/ox-alpha" });
					res.writeHead(200, { "Content-Type": "application/json" });
					res.end(JSON.stringify({ ok: true, configured: true, defaultModel: { provider: "openrouter", model: "stealth/ox-alpha" } }));
				} catch (e) {
					res.writeHead(400, { "Content-Type": "application/json" });
					res.end(JSON.stringify({ ok: false, error: String((e && e.message) || e) }));
				}
