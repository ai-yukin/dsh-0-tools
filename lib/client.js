			...
			return new Promise((resolve) => {
				dshApi("settings.describe", {}).then((desc) => {
					...
					return probeHealthProxy().then((proxyPort) => {
						if (!proxyPort) {
							// 代理不可用：浏览器端没有 API Key 明文，无法直接 fetch，只能返回 null
							resolve(null);
							return;
						}
						// 通过本地代理测速（代理服务器端从 ~/.dsh/.credentials.yaml 读取 API Key）
						return fetch("http://127.0.0.1:" + proxyPort + "/ping", {
							method: "POST",
							headers: { "Content-Type": "application/json" },
							body: JSON.stringify({ baseURL: block.baseURL, modelId: modelId, apiKeyEnv: block.apiKeyEnv }),
