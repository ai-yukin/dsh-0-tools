...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function FooterActionsWrapper(props) {
			...
			// 设置弹窗监听：打开设置时默认跳零号工具标签
			_react.useEffect(() => {
				let observer = null;
				try {
					observer = new MutationObserver(() => {
						// 检测设置弹窗是否打开
						const dialog = document.querySelector('[role="dialog"], [class*="dialog"], [class*="modal"]');
						if (dialog) {
							// 找到零号工具标签并点击
							setTimeout(() => {
								const labels = document.querySelectorAll('[class*="navLabel"]');
								for (const el of labels) {
									if ((el.textContent || "").trim() === "零号工具") {
										const cell = el.closest('[class*="navCell"]');
										if (cell && !cell.classList.contains("dsh-0-tools-auto-navigated")) {
											cell.classList.add("dsh-0-tools-auto-navigated");
											cell.click();
										}
										break;
									}
