	...
	factory: (require) => {
		...
		function switchCurrentConversationModel(provider) {
			...
			return new Promise((resolve, reject) => {
				try {
					// 辅助函数：触发元素的鼠标事件（兼容 React 事件系统）
					function triggerClick(el) {
						if (!el) return false;
						console.log("[dsh-0-tools] 触发点击:", el.tagName, el.className);
						try {
							// 先触发 mousedown（有些组件用 mousedown 而不是 click）
							...
							console.warn("[dsh-0-tools] dispatchEvent 失败，尝试 .click():", e);
							try {
								el.click();
								return true;
							} catch (e2) {
								console.warn("[dsh-0-tools] .click() 也失败:", e2);
								return false;
							}
						}
					}
					...
					// 1. 找到模型选择器触发按钮
					const triggerLabel = document.querySelector("._7KE1Ra_triggerLabel");
					if (!triggerLabel) {
						reject(new Error("未找到模型选择器触发按钮 ._7KE1Ra_triggerLabel"));
						return;
					}
					console.log("[dsh-0-tools] 找到触发按钮文本:", triggerLabel.textContent);

					// 2. 点击触发按钮（尝试点击 label 本身，以及向上找的按钮）
					let clickSuccess = false;
					// 先尝试点击 label 本身
					if (triggerClick(triggerLabel)) {
						clickSuccess = true;
						console.log("[dsh-0-tools] 点击 triggerLabel 成功");
					}
					// 如果没成功，向上找按钮元素
					if (!clickSuccess) {
						let parent = triggerLabel.parentElement;
						let depth = 0;
						while (parent && depth < 5) {
							if (parent.tagName === "BUTTON" || parent.getAttribute("role") === "button") {
								if (triggerClick(parent)) {
									clickSuccess = true;
									console.log("[dsh-0-tools] 点击父级按钮成功:", parent.tagName, parent.className);
									break;
								}
							}
					...
					if (!clickSuccess) {
						reject(new Error("无法点击模型选择器触发按钮"));
						return;
					}

					// 3. 等待下拉列表出现（重试5次，每次500ms）
					let retryCount = 0;
					const maxRetries = 5;
							...
							try {
								...
								if (groups.length === 0) {
									...
									if (retryCount < maxRetries) {
										...
										triggerClick(triggerLabel);
										tryFindAndClick();
									} else {
										reject(new Error("下拉列表未出现，重试" + maxRetries + "次失败"));
									}
									return;
								}
								...
								for (const group of groups) {
									...
									// 匹配方式：aria-labelledby 包含 "-provider" 或者分组标题包含 provider 名
									if (ariaLabelledby.includes("-" + provider) || ariaLabelledby.includes(provider)) {
										targetGroup = group;
										break;
									}
								}
								...
								// 如果没找到，尝试通过分组标题匹配
								if (!targetGroup) {
									console.log("[dsh-0-tools] 通过 aria-labelledby 未找到，尝试通过分组标题匹配");
									const providerNames = {
										"openrouter-free": ["OpenRouter", "openrouter"],
										"zai": ["智谱", "zai", "ZAI"],
										"siliconflow": ["硅基流动", "siliconflow", "SiliconFlow"],
										"xinghuo": ["讯飞", "星火", "xinghuo", "Xinghuo"]
									};
									const names = providerNames[provider] || [provider];
									for (const group of groups) {
										const titleEl = group.querySelector("._7KE1Ra_groupTitle");
										const title = titleEl ? titleEl.textContent : "";
										console.log("[dsh-0-tools] 分组标题:", title);
								...
								if (!targetGroup) {
									reject(new Error("未找到模型分组: " + provider));
									return;
								}
								...
								// 6. 点击模型选项
								const optionClickSuccess = triggerClick(option);
								if (!optionClickSuccess) {
									// 尝试点击选项里的 modelName 元素
									if (modelName && triggerClick(modelName)) {
										console.log("[dsh-0-tools] 点击 modelName 成功");
									} else {
										reject(new Error("无法点击模型选项"));
										return;
									}
								}
