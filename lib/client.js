...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const guideUrl = (type) => (_guideUseGitee ? GITEE_GUIDE_BASE : GITHUB_GUIDE_BASE) + type + ".html";
		// 启动时检测Gitee Pages可用性（测试help.json，因为和guide页面在同一个Pages服务下）
		fetch("https://ai-yukin.gitee.io/dsh-0-tools/help.json", { method: "HEAD", mode: "no-cors" })
			.then(() => { _guideUseGitee = true; })
			.catch(() => { _guideUseGitee = false; });
		...
		function OnboardingModal(props) {
			...
			const guideUrl = (item) => guideUrl(item.type);
						...
						children: [
							...
							(0, react_jsx_runtime.jsx)("div", {
								...
								children: list.map((m) => {
									...
									const confirming = uninstallTarget === m.type;
									const guideUrl = guideUrl(m.type);
									return (0, react_jsx_runtime.jsxs)("div", {
										key: m.type,
										style: {
											display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "space-between",
