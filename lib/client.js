...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		let _healthAutoMode = false; // 自动切换模式（默认关闭）
		...
		function OnboardingModal(props) {
			...
			// 模型显示名映射（从 freeModels 的 title 提取）
			const getModelName = (item) => {
				if (item.title) {
					// 从 title 中提取模型名（去掉"一键配置"等后缀）
					...
					children: [
						(0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								"建议配齐以下所有免费模型，免费模型数越多，切换选择更多！点击「查看教程」获取 API Key，复制后回到 DSH 将自动识别并一键配置。"
							]
						}),
						(0, react_jsx_runtime.jsx)("div", {
		...
		function FreeConfigCenter(props) {
			...
			const [switching, setSwitching] = _react.useState(false);
			const [switchMsg, setSwitchMsg] = _react.useState("");
			...
			const fastest = health ? health.getFastest() : null;
				...
				children: [
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsx)("div", {
								...
								children: "建议配齐以下所有免费模型，免费模型数越多，切换选择更多！单个模型故障时自动绕开，更稳定。"
							}),
							// 模型卡片网格（只显示已接入/未配置，不显示速度对比）
							(0, react_jsx_runtime.jsx)("div", {
									...
									return (0, react_jsx_runtime.jsxs)("div", {
										...
										children: [
											...
											isConfigured
												...
												: (0, react_jsx_runtime.jsx)("a", { href: guideUrl, target: "_blank", rel: "noreferrer", style: { fontSize: "10px", color: "#2563eb", fontWeight: 600, textDecoration: "none", cursor: "pointer" }, children: "获取Key →" })
										]
									});
								})
							}),
							// 自动切换开关（保留，加解释）
							(0, react_jsx_runtime.jsxs)("div", {
								style: { display: "flex", alignItems: "center", gap: "8px", marginTop: "10px", paddingTop: "10px", borderTop: "1px solid #eee" },
								children: [
									(0, react_jsx_runtime.jsxs)("label", {
										style: { display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", color: "#555", cursor: "pointer" },
										children: [
											(0, react_jsx_runtime.jsx)("input", {
												type: "checkbox",
												checked: health ? health.autoMode : false,
												onChange: (e) => health && health.setAutoMode(e.target.checked),
												style: { cursor: "pointer" }
											}),
											(0, react_jsx_runtime.jsx)("span", { children: "自动切换" })
										]
									}),
									(0, react_jsx_runtime.jsx)("span", { style: { fontSize: "10px", color: "#999" }, children: "（当前模型连续2次不可用时，自动换到其他可用的免费模型）" })
								]
							})
						]
					}),
					(0, react_jsx_runtime.jsx)("div", {
						...
						children: [
							...
							" DeepSeek 官方 API 无永久免费模型，需充值才能调用；这里汇总当前可一键接入的免费模型，零费用先用起来，随时可一键卸载。接入多个后插件会自动监测健康状态，帮你选最稳定的。"
						]
					}),
					(0, react_jsx_runtime.jsx)("div", {
						style: { fontSize: "12px", lineHeight: "18px", color: "#61666B" },
						children: installedCount > 0
							? "已接入 " + installedCount + " 个免费模型，点击下方条目可查看详情或卸载。"
							: "暂未接入免费模型，点击上方「获取Key」查看教程，复制Key后回到DSH将自动识别并一键配置。"
					}),
		...
		function FooterActions(props) {
			...
			const collapsed = useSidebarCollapsed();
			// v1.8.0: 未配置免费模型时显示"🔑 配置免费模型"小入口（点击弹引导弹窗）
			const showConfigEntry = loaded ? !hasAnyInstalled : false;
		...
		function FooterActionsWrapper(props) {
			...
			// v1.8.0: 引导弹窗打开时监听剪贴板，检测 Key 并判断所属模型
			_react.useEffect(() => {
				if (!showOnboarding) return; // 只在引导弹窗打开时监听
				let timer = null;
				const sniff = () => {
					try {
						navigator.clipboard.readText().then((text) => {
							const raw = (text || "").trim();
							if (!raw || raw.length < 16 || raw.length > 200) return;
							// 根据 Key 格式判断所属模型
							let matchedType = null;
							if (raw.indexOf("sk-or-v1-") === 0) matchedType = "openrouter-free";
							else if (raw.indexOf("bce-v3/") === 0) matchedType = "qianfan";
							else if (/^[0-9a-f]{32}$/i.test(raw)) matchedType = "xinghuo"; // 讯飞 Key 通常是32位十六进制
							else if (/^[a-zA-Z0-9]{20,}$/.test(raw) && raw.length >= 24) matchedType = "zai"; // 智谱 Key 格式
							// 如果无法判断，静默忽略（避免误判）
							if (!matchedType) return;
							// 检查是否已经配置过
							if (configuredStatus && configuredStatus[matchedType]) return;
							// 检查是否已经嗅探到同一个 Key（避免重复提示）
							if (sniffedKey === raw) return;
							setSniffedKey(raw);
							setSniffedModel(matchedType);
							setConfigMsg("");
						}).catch(() => { /* 用户拒绝授权或不支持，静默忽略 */ });
					} catch (e) { /* ignore */ }
				};
				timer = setInterval(sniff, 2000); // 每2秒检测一次
				return () => { if (timer) clearInterval(timer); };
			}, [showOnboarding, configuredStatus, sniffedKey]);
