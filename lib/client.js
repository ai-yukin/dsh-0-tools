...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ModelStatusIndicator(props) {
			...
			const statusLabel = s.status === "ok" ? "正常" : (s.status === "slow" ? "较慢" : "响应超时");
			...
			const handleClick = () => {
				...
				setSwitching(true);
				setMsg("正在切换…");
				// v1.8.5: 先切换默认模型到最快可用，再刷新页面让当前对话也切换
				health.switchFastest().then(() => {
					setMsg("✅ 已切换，正在刷新…");
					...
					(0, react_jsx_runtime.jsxs)("button", {
						...
						children: [
							(0, react_jsx_runtime.jsxs)("span", { style: { display: "flex", alignItems: "center", gap: "4px" }, children: [
								...
								(0, react_jsx_runtime.jsx)("span", { style: { fontWeight: 600 }, children: switching ? "切换中…" : (s.status === "unavailable" ? currentModelName + "响应超时🔄点击切换更快模型" : (slowWarning ? currentModelName + "·" + statusLabel + "，点此切换" : currentModelName + " · " + statusLabel)) })
							]}),
						]
					}),
