...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
			...
			const submit = () => {
				...
				readAgentDefault()
					...
					.then(() => dshApi("settings.mutate", {
						ns: "llm-pi-ai",
						ops: [{ op: "set", path: ["providers", item.type], value: { apiKeyEnv: credRef, models: models } }]
					}))
		...
		function remoteFilterModel(m) {
			...
			return { id: m.id, name: m.name, input: input };
		}
		...
		function FooterActions(props) {
			const { wide } = props;
			const { status, loaded } = useConfigured();
			// 提示条显隐：仅当智谱免费模型未配置时显示；loaded 门控避免首次
			// 状态加载完成前闪现引导按钮（status 初始为空对象）。
			const showHint = loaded ? !status.zai : false;
