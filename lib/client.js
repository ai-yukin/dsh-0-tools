	...
	factory: (require) => {
		...
		function pingProvider(providerKey) {
			return new Promise((resolve) => {
				console.log("[dsh-0-tools health] pingProvider start:", providerKey);
				dshApi("settings.describe", {}).then((desc) => {
					const ns = desc && desc.namespaces ? desc.namespaces.find((n) => n.ns === "llm-pi-ai") : null;
					const block = ns && ns.user && ns.user.providers && ns.user.providers[providerKey];
					console.log("[dsh-0-tools health] settings.describe block:", providerKey, block ? { baseURL: block.baseURL, apiKeyEnv: block.apiKeyEnv, models: block.models } : "null");
					if (!block || !block.baseURL || !block.apiKeyEnv) {
						console.log("[dsh-0-tools health] resolve null: missing block/baseURL/apiKeyEnv");
						resolve(null);
						return;
					}
					const modelId = block.models && block.models[0] && block.models[0].id;
					console.log("[dsh-0-tools health] modelId:", modelId);
					if (!modelId) { console.log("[dsh-0-tools health] resolve null: missing modelId"); resolve(null); return; }

					return probeHealthProxy().then((proxyPort) => {
						console.log("[dsh-0-tools health] proxyPort:", proxyPort);
						if (!proxyPort) {
							console.log("[dsh-0-tools health] resolve null: proxyPort null");
							resolve(null);
							return;
						}
						const reqBody = { baseURL: block.baseURL, modelId: modelId, apiKeyEnv: block.apiKeyEnv };
						console.log("[dsh-0-tools health] fetch proxy reqBody:", reqBody);
						return fetch("http://127.0.0.1:" + proxyPort + "/ping", {
							...
							console.log("[dsh-0-tools health] proxy resp:", data);
							if (data && data.ok && typeof data.latency === "number") {
								console.log("[dsh-0-tools health] resolve latency:", data.latency);
								resolve(data.latency);
							} else {
								console.log("[dsh-0-tools health] resolve null: resp not ok");
								resolve(null);
							}
						}).catch((e) => {
							console.log("[dsh-0-tools health] fetch error:", e);
							_healthProxyPort = null;
							resolve(null);
						});
					});
				}).catch((e) => { console.log("[dsh-0-tools health] settings.describe error:", e); resolve(null); });
			});
		}
