...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const BUILTIN_PROVIDER_SPECS = {
			...
			// v1.8.0：百度千帆 ERNIE-Speed-8K（永久免费，QPS=50）。
			"qianfan": {
				...
				models: [
					{ id: "ernie-speed-8k", name: "百度 ERNIE-Speed-8K（永久免费）", input: ["text"], contextWindow: 8000, maxTokens: 2048 }
				]
			},
		...
		function loadHealthAutoMode() {
			try { _healthAutoMode = localStorage.getItem(HEALTH_AUTO_KEY) === "1"; } catch (e) { _healthAutoMode = false; }
		}
		...
		// 对单个 provider 发一次测速请求（max_tokens=1）
		function pingProvider(providerKey) {
			return new Promise((resolve) => {
				// 从 DSH 配置读取该 provider 的 baseURL 和 apiKey
				dshApi("settings.describe", {}).then((desc) => {
					...
					// 读取凭据
					return dshApi("credentials.describe", { refs: [block.apiKeyEnv] }).then((cred) => {
						const credMap = (cred && cred.credentials) || {};
						const apiKey = credMap[block.apiKeyEnv] && credMap[block.apiKeyEnv].value;
						if (!apiKey) { resolve(null); return; }
						// 发极小请求测速
						const start = Date.now();
						const ctrl = new AbortController();
						const timer = setTimeout(() => ctrl.abort(), HEALTH_TIMEOUT);
						return fetch(block.baseURL + "/chat/completions", {
							method: "POST",
							...
							clearTimeout(timer);
							if (r.ok) resolve(Date.now() - start);
							else resolve(null);
						}).catch(() => {
							clearTimeout(timer);
							resolve(null);
		...
		function runHealthCheck() {
			...
			return readProviderStatus().then((status) => {
				...
				return Promise.all(configured.map((p) => pingProvider(p).then((latency) => {
					...
					// v1.8.0: 自动切换——当前默认模型连续2次不可用时，自动切到最快可用模型
					if (!_healthAutoMode || configured.length < 2) return;
					return getCurrentDefaultModel().then((cur) => {
						if (!cur) return;
						const s = _healthState[cur.provider];
						if (s && s.status === "unavailable" && (s.consecutiveUnavailable || 0) >= AUTO_SWITCH_THRESHOLD) {
							return switchToFastest().then((best) => {
								if (best && best.provider !== cur.provider) {
									notifyAutoSwitched(cur.provider, best.provider);
								}
							}).catch(() => { /* 切换失败忽略 */ });
						}
					}).catch(() => { /* ignore */ });
		...
		// v1.8.0: 自动切换通知——被切换后提示用户（模块级回调，UI 通过 setAutoSwitchNotify 订阅）
		let _autoSwitchNotifier = null;
		function setAutoSwitchNotify(cb) { _autoSwitchNotifier = cb; }
		function notifyAutoSwitched(fromProvider, toProvider) {
			if (_autoSwitchNotifier) {
				_autoSwitchNotifier("当前模型较慢，已切换新模型");
			}
		}
		...
		const GUIDE_BASE_URL = "https://ai-yukin.github.io/dsh-0-tools/guide/";

		const GUIDE_MODELS = [
			{ key: "zai", name: "智谱 GLM", desc: "文本+图片理解，永久免费", color: "#2563eb", url: GUIDE_BASE_URL + "zai.html" },
			{ key: "qianfan", name: "百度千帆", desc: "ERNIE-Speed，QPS=50 高并发", color: "#2563eb", url: GUIDE_BASE_URL + "qianfan.html" },
			{ key: "xinghuo", name: "讯飞星火", desc: "Spark Lite，中文理解强", color: "#2563eb", url: GUIDE_BASE_URL + "xinghuo.html" },
			{ key: "openrouter-free", name: "OpenRouter", desc: "免费池自动挑选数十款", color: "#2563eb", url: GUIDE_BASE_URL + "openrouter.html" }
		];

		function OnboardingModal(props) {
			const { onClose, freeModels, configuredStatus, onConfigured } = props;
			...
			const getModelDesc = (item) => item.desc ? item.desc.slice(0, 40) + "…" : "";
				...
				children: (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						(0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								(0, react_jsx_runtime.jsx)("span", { style: { fontWeight: 600, color: "#111" }, children: "已配置 " + configuredCount + "/" + totalCount + " 个免费模型。" }),
								"建议配齐以下所有免费模型，免费模型数越多，切换选择更多！点击以下模型卡片获取 API Key，复制后将自动识别并一键配置。"
							]
						}),
						(0, react_jsx_runtime.jsx)("div", {
							...
							children: models.map((m) => {
								...
								const guideUrl = GUIDE_BASE_URL + m.type + ".html";
								return (0, react_jsx_runtime.jsxs)("div", {
									key: m.type,
									style: {
										display: "flex", flexDirection: "column", alignItems: "flex-start", gap: "4px",
										padding: "14px", borderRadius: "12px", border: "1px solid " + (isConfigured ? "#c8e6c9" : "#e0e0e8"),
										background: isConfigured ? "#f1f8f2" : "#fafbfc", cursor: "pointer", textAlign: "left",
										transition: "all 0.15s"
									},
									onClick: () => openExternal(guideUrl),
									children: [
										(0, react_jsx_runtime.jsxs)("span", { style: { display: "flex", alignItems: "center", gap: "4px" }, children: [
											...
											isConfigured ? (0, react_jsx_runtime.jsx)("span", { style: { fontSize: "10px", color: "#2e7d32", fontWeight: 700, background: "#c8e6c9", padding: "1px 6px", borderRadius: "8px" }, children: "已接入" }) : null
										]}),
										(0, react_jsx_runtime.jsx)("span", { style: { fontSize: "11px", color: "#888", lineHeight: "1.4" }, children: getModelDesc(m) }),
										(0, react_jsx_runtime.jsx)("span", { style: { fontSize: "11px", color: "#2563eb", fontWeight: 600, marginTop: "2px" }, children: isConfigured ? "查看教程 →" : "获取 Key →" })
									]
								});
							})
						}),
						// v1.8.0: 用户主动触发剪贴板识别（避免持续监听吓到小白）
						(0, react_jsx_runtime.jsxs)("div", {
							style: { display: "flex", flexDirection: "column", gap: "6px", marginTop: "4px" },
							children: [
								(0, react_jsx_runtime.jsx)("button", {
									type: "button",
									onClick: onSniffClipboard,
									style: {
										...BTN_PRIMARY,
										width: "100%",
										padding: "10px",
									...
									children: "📋 我已复制好 Key了，自动识别配置"
								}),
								sniffMsg ? (0, react_jsx_runtime.jsx)("div", {
									style: {
										fontSize: "11px",
										color: sniffMsg.indexOf("⚠️") === 0 || sniffMsg.indexOf("ℹ️") === 0 ? "#f5a623" : "#30a46c",
										textAlign: "center",
										lineHeight: "1.4"
									},
									children: sniffMsg
		...
		function ModelStatusIndicator(props) {
			...
			const statusLabel = s.status === "ok" ? "正常" : (s.status === "slow" ? "较慢" : "不可用");
			...
			const handleClick = () => {
				...
				health.switchFastest().then((result) => {
					setSwitching(false);
					setMsg("✅ 已切换到最快模型（" + result.avgLatency + "ms）");
					onSwitch && onSwitch();
				...
				children: [
					(0, react_jsx_runtime.jsxs)("button", {
						...
						style: {
							...
							border: "1px solid " + (slowWarning ? "#f5d7a0" : "#c8e6c9"),
							background: slowWarning ? "#fff8e1" : "#f1f8f2",
							cursor: switching ? "wait" : "pointer", fontSize: "11px",
							color: "#333", transition: "all 0.15s"
						},
						children: [
							(0, react_jsx_runtime.jsxs)("span", { style: { display: "flex", alignItems: "center", gap: "4px" }, children: [
								(0, react_jsx_runtime.jsx)("span", { children: statusText }),
								(0, react_jsx_runtime.jsx)("span", { style: { fontWeight: 600 }, children: switching ? "切换中…" : (slowWarning ? "当前模型" + statusLabel + "，点此切换" : "当前模型 · " + statusLabel) })
							]}),
							(0, react_jsx_runtime.jsx)("span", { style: { color: "#2563eb", fontWeight: 600 }, children: "🚀 切最快" })
						]
					}),
		...
		const DEFAULT_HELP = {
			...
			freeModels: [
				...
				{
					type: "qianfan",
					title: "百度千帆 ERNIE-Speed-8K（永久免费，QPS=50）",
					desc: "百度千帆 ERNIE-Speed-8K 永久免费不限量，QPS 高达 50（比智谱免费版并发高 50 倍），国内访问速度快，适合高频率日常使用。粘贴 API Key 一键接入。",
					keysLabel: "百度千帆控制台获取 Key ↗",
					keysUrl: "https://console.bce.baidu.com/qianfan/",
					keyPlaceholder: "粘贴你的百度千帆 API Key（bce-v3/ 开头）",
					keyPrefix: "bce-v3/",
					credentialRef: "QIANFAN_API_KEY",
					displayName: "百度千帆",
					api: "openai-completions",
					baseURL: "https://qianfan.baidubce.com/v2",
					models: [
					...
					note: "需零号工具 v1.8.0 及以上版本。百度千帆免费模型 QPS=50，并发能力强，适合作为主力免费模型。"
				},
				{
					type: "xinghuo",
					title: "讯飞星火 Spark Lite（永久免费，2QPS）",
					desc: "讯飞星火 Spark Lite 永久免费，中文语义理解和多轮对话能力强，国内访问稳定。粘贴 API Key 一键接入，作为免费模型池的补充。",
					keysLabel: "讯飞星火控制台获取 Key ↗",
					...
					note: "需零号工具 v1.8.0 及以上版本。讯飞星火 Lite 免费版限速 2QPS，适合作为备用免费模型。"
				}
			],
		...
		function FreeConfigCenter(props) {
			...
			// v1.8.0: 已配置的模型展开显示详情/卸载；未配置的点击跳教程
			const [expanded, setExpanded] = _react.useState(null);
			// v1.8.0: 手动配置兜底（默认折叠，点击展开）
			const [showManual, setShowManual] = _react.useState(false);

			// 模型显示名映射：优先用短名（智谱/百度/OpenRouter/讯飞），无短名时回退 title
			const getModelName = (item) => {
				if (item.shortName) return item.shortName;
				if (item.title) {
					return item.title.replace(/（.*$/g, "").replace(/一键配置.*/g, "").trim();
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				style: { display: "flex", flexDirection: "column", gap: "12px" },
				children: [
					// v1.8.0: 配置进度提示（不显示速度对比，引导配齐所有模型）
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							(0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									(0, react_jsx_runtime.jsxs)("span", { style: { fontSize: "13px", fontWeight: 700, color: "#111" }, children: [
										(0, react_jsx_runtime.jsx)("span", { children: "已配置 " + installedCount + "/" + totalCount + " 个免费模型" }),
										installedCount >= totalCount && totalCount > 0 ? (0, react_jsx_runtime.jsx)("span", { style: { marginLeft: "8px", fontSize: "12px", color: "#2e7d32" }, children: "🎉 已配齐全部免费模型" }) : null
									]}),
									(0, react_jsx_runtime.jsx)("button", {
										type: "button",
										onClick: () => refresh(),
										style: { border: "none", background: "transparent", color: "#2563eb", fontSize: "11px", cursor: "pointer", padding: "2px 6px" },
										children: "🔄 刷新状态"
									})
								]
							}),
							(0, react_jsx_runtime.jsx)("div", {
								...
								children: "建议配齐以下所有免费模型，免费模型数越多，切换选择更多！"
							}),
							// 模型卡片网格（只显示已接入/未配置，不显示速度对比；已配置可展开管理）
							(0, react_jsx_runtime.jsx)("div", {
								style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px", marginTop: "10px" },
								children: list.map((m) => {
									const isConfigured = !!status[m.type];
									const isExpanded = expanded === m.type;
									const guideUrl = "https://ai-yukin.github.io/dsh-0-tools/guide/" + m.type + ".html";
									return (0, react_jsx_runtime.jsxs)("div", {
										key: m.type,
										style: {
											display: "flex", flexDirection: "column",
											padding: "8px 10px", borderRadius: "8px",
											background: isConfigured ? "#f1f8f2" : "#fff",
											border: "1px solid " + (isConfigured ? "#c8e6c9" : "#e8e8e8"),
											cursor: isConfigured ? "pointer" : "default"
										},
										onClick: isConfigured ? () => setExpanded(isExpanded ? null : m.type) : undefined,
										children: [
											(0, react_jsx_runtime.jsxs)("div", {
												style: { display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%" },
												children: [
													(0, react_jsx_runtime.jsxs)("span", { style: { display: "flex", alignItems: "center", gap: "4px", fontSize: "11px" }, children: [
														(0, react_jsx_runtime.jsx)("span", { children: isConfigured ? "✅" : "🔲" }),
														(0, react_jsx_runtime.jsx)("span", { style: { fontWeight: 600, color: "#333" }, children: getModelName(m) })
													]}),
													isConfigured
														? (0, react_jsx_runtime.jsx)("span", { style: { fontSize: "10px", color: "#2e7d32", fontWeight: 700 }, children: "已接入 ▾" })
														: (0, react_jsx_runtime.jsx)("a", { href: guideUrl, target: "_blank", rel: "noreferrer", style: { fontSize: "10px", color: "#2563eb", fontWeight: 600, textDecoration: "none", cursor: "pointer" }, children: "引导教程 →" })
												]
											}),
											isExpanded ? (0, react_jsx_runtime.jsx)("div", {
												style: { marginTop: "8px", paddingTop: "8px", borderTop: "1px dashed #c8e6c9", width: "100%" },
												children: (0, react_jsx_runtime.jsx)(FreeInstalledZone, { item: m, onDone: refresh })
											}) : null
										]
									});
								})
							}),
							// v1.8.0: 手动配置兜底入口（默认折叠隐藏）
							(0, react_jsx_runtime.jsxs)("div", {
								style: { marginTop: "8px" },
								children: [
									(0, react_jsx_runtime.jsx)("button", {
										...
										style: { border: "none", background: "transparent", color: "#81858C", fontSize: "13px", fontWeight: 700, cursor: "pointer", textDecoration: "underline", padding: "4px 0" },
										children: showManual ? "收起手动配置 ▴" : "🛠 手动配置（粘贴Key） ▾"
									}),
									showManual ? (0, react_jsx_runtime.jsx)("div", {
										style: { display: "flex", flexDirection: "column", gap: "10px", marginTop: "8px" },
										children: list.map((item) => (0, react_jsx_runtime.jsx)("div", {
											style: { border: "1px solid #e8e8e8", borderRadius: "8px", padding: "8px" },
											children: (0, react_jsx_runtime.jsx)(FreeInstallForm, { item: item, onDone: refresh })
										}, "manual-" + item.type))
									}) : null
								]
							})
		...
		function FooterActions(props) {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					showConfigEntry ? (0, react_jsx_runtime.jsx)("button", {
						type: "button",
						onClick: onOpenOnboarding,
						style: {
							display: "flex", alignItems: "center", gap: "4px",
							padding: "5px 10px", borderRadius: "8px",
							border: "1px solid #c8e6c9", background: "#f1f8f2",
							cursor: "pointer", fontSize: "11px", fontWeight: 600,
							color: "#2e7d32", transition: "all 0.15s"
						},
						onMouseEnter: (e) => { e.currentTarget.style.background = "#e8f5e9"; },
						onMouseLeave: (e) => { e.currentTarget.style.background = "#f1f8f2"; },
						children: "🔑 配置免费模型"
					}) : null,
		...
		function FooterActionsWrapper(props) {
			...
			const { loaded, hasAnyInstalled, status: configuredStatus } = useConfigured();
			...
			// v1.8.0: Key 嗅探状态（只在引导弹窗打开时监听剪贴板）
			const [sniffedKey, setSniffedKey] = _react.useState(null);
			const [sniffedModel, setSniffedModel] = _react.useState(null);
			const [configuring, setConfiguring] = _react.useState(false);
			const [configMsg, setConfigMsg] = _react.useState("");
			// v1.8.0: 自动切换通知消息
			const [autoSwitchMsg, setAutoSwitchMsg] = _react.useState("");
			...
			// v1.8.0: 订阅自动切换通知（被切换后提示用户）
			_react.useEffect(() => {
				setAutoSwitchNotify((msg) => {
					setAutoSwitchMsg(msg);
					setTimeout(() => setAutoSwitchMsg(""), 5000);
				});
				return () => setAutoSwitchNotify(null);
			}, []);

			// v1.8.0: 用户主动点击"我已复制好Key"时读取剪贴板识别（不持续监听，避免权限弹窗吓到小白）
			const sniffClipboard = () => {
				try {
					navigator.clipboard.readText().then((text) => {
						const raw = (text || "").trim();
						if (!raw || raw.length < 16 || raw.length > 200) {
							setConfigMsg("⚠️ 剪贴板中没有检测到有效的 API Key，请先复制 Key 再点击");
							setTimeout(() => setConfigMsg(""), 3000);
							return;
						}
						// 根据 Key 格式判断所属模型
						let matchedType = null;
						if (raw.indexOf("sk-or-v1-") === 0) matchedType = "openrouter-free";
						else if (raw.indexOf("bce-v3/") === 0) matchedType = "qianfan";
						else if (/^[0-9a-f]{32}$/i.test(raw)) matchedType = "xinghuo";
						else if (/^[a-zA-Z0-9]{20,}$/.test(raw) && raw.length >= 24) matchedType = "zai";
						// 无法判断时提示用户
						if (!matchedType) {
							setConfigMsg("⚠️ 无法识别这个 Key 属于哪个模型，请检查是否复制正确");
							setTimeout(() => setConfigMsg(""), 3000);
							return;
						}
						// 已配置过则提示
						if (configuredStatus && configuredStatus[matchedType]) {
							const mName = matchedType === "openrouter-free" ? "OpenRouter" : matchedType === "qianfan" ? "百度千帆" : matchedType === "xinghuo" ? "讯飞星火" : matchedType === "zai" ? "智谱 GLM" : matchedType;
							setConfigMsg("ℹ️ 「" + mName + "」已经配置过了，无需重复配置");
							setTimeout(() => setConfigMsg(""), 3000);
							return;
						}
						setSniffedKey(raw);
						setSniffedModel(matchedType);
						setConfigMsg("");
					}).catch(() => {
						setConfigMsg("⚠️ 无法读取剪贴板，请在浏览器弹窗中允许剪贴板权限，或手动粘贴配置");
						setTimeout(() => setConfigMsg(""), 4000);
					});
				} catch (e) {
					setConfigMsg("⚠️ 剪贴板读取失败，请手动粘贴配置");
					setTimeout(() => setConfigMsg(""), 3000);
				}
			};

			// 设置弹窗监听：打开设置时默认跳零号工具标签（v1.8.0修正：用更通用的方式查找标签）
			_react.useEffect(() => {
				let observer = null;
				let navigated = false;
				const tryNavigate = () => {
					if (navigated) return;
					// 用更通用的方式：遍历所有叶子元素，找到文本为"零号工具"的元素
					const all = document.querySelectorAll("div, span, button, a, p, li");
					for (const el of all) {
						if (el.children.length > 0) continue; // 只看叶子节点
						const text = (el.textContent || "").trim();
						if (text !== "零号工具") continue;
						// 向上找到可点击的父元素
						let clickable = el;
						let depth = 0;
						while (clickable && clickable.tagName !== "BODY" && depth < 10) {
							const tag = clickable.tagName;
							const role = clickable.getAttribute("role");
							const style = window.getComputedStyle(clickable);
							if (tag === "BUTTON" || tag === "A" || role === "tab" || role === "menuitem" || style.cursor === "pointer") {
								if (!clickable.classList.contains("dsh-0-tools-auto-navigated")) {
									clickable.classList.add("dsh-0-tools-auto-navigated");
									navigated = true;
									// 用 mousedown + mouseup + click 确保 React 事件触发
						...
						// 如果没找到可点击父元素，直接点击当前元素
						if (!el.classList.contains("dsh-0-tools-auto-navigated")) {
							el.classList.add("dsh-0-tools-auto-navigated");
							navigated = true;
			...
			// v1.8.0: 嗅探到 Key 后一键自动配置
			const autoConfigureModel = () => {
				if (!sniffedKey || !sniffedModel || configuring) return;
				setConfiguring(true);
				setConfigMsg("");
				const modelType = sniffedModel;
				const apiKey = sniffedKey;
				// 找到该模型的配置信息
				const modelItem = (helpData && helpData.freeModels) ? helpData.freeModels.find((m) => m.type === modelType) : null;
				...
				readAgentDefault()
					...
					.then(() => {
						setConfiguring(false);
						const modelName = modelItem ? (modelItem.title || modelType) : modelType;
						const configuredNow = (helpData && helpData.freeModels) ? helpData.freeModels.filter((m) => m.type === modelType || (configuredStatus && configuredStatus[m.type])).length : 1;
						setConfigMsg("✅ 已自动配置「" + modelName + "」，已接入 " + configuredNow + "/" + (helpData && helpData.freeModels ? helpData.freeModels.length : "?") + " 个免费模型");
						setSniffedKey(null);
						setSniffedModel(null);
						// 刷新配置状态
						setTimeout(() => {
							getCurrentDefaultModel().then((m) => setCurrentProvider(m ? m.provider : null)).catch(() => {});
						}, 500);
			...
			return (0, react_jsx_runtime.jsxs)(_react.Fragment, {
				children: [
					...
					showOnboarding ? (0, react_jsx_runtime.jsx)(OnboardingModal, {
						...
						onSniffClipboard: sniffClipboard,
						sniffMsg: configMsg
					}) : null,
					// v1.8.0: 自动切换通知（右下角浮动提示）
					autoSwitchMsg ? (0, react_jsx_runtime.jsxs)("div", {
						style: {
							position: "fixed", bottom: "80px", right: "20px", zIndex: 99999,
							background: "#fff7ed", borderRadius: "12px", boxShadow: "0 8px 32px rgba(0,0,0,0.2)",
							padding: "12px 16px", maxWidth: "320px", border: "1px solid #fdba74"
						},
						children: [
							(0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", alignItems: "center", gap: "6px" }, children: [
								(0, react_jsx_runtime.jsx)("span", { style: { fontSize: "16px" }, children: "🔄" }),
								(0, react_jsx_runtime.jsx)("span", { style: { fontSize: "12px", color: "#9a3412", lineHeight: "1.5" }, children: autoSwitchMsg })
							]})
						]
					}) : null,
					// v1.8.0: Key 嗅探浮动提示（右下角）
					sniffedModel ? (0, react_jsx_runtime.jsxs)("div", {
						style: {
							position: "fixed", bottom: "20px", right: "20px", zIndex: 99999,
							background: "#fff", borderRadius: "12px", boxShadow: "0 8px 32px rgba(0,0,0,0.2)",
							padding: "14px 16px", maxWidth: "320px", border: "1px solid #e0e0e8"
						},
						children: [
							(0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", flexDirection: "column", gap: "8px" }, children: [
								(0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", alignItems: "center", gap: "6px" }, children: [
									(0, react_jsx_runtime.jsx)("span", { style: { fontSize: "16px" }, children: "📋" }),
									(0, react_jsx_runtime.jsx)("span", { style: { fontSize: "13px", fontWeight: 700, color: "#111" }, children: "已检测到 API Key" })
								]}),
								(0, react_jsx_runtime.jsx)("div", { style: { fontSize: "12px", color: "#555", lineHeight: "1.5" }, children: "识别为「" + (sniffedModel === "openrouter-free" ? "OpenRouter" : sniffedModel === "qianfan" ? "百度千帆" : sniffedModel === "xinghuo" ? "讯飞星火" : sniffedModel === "zai" ? "智谱 GLM" : sniffedModel) + "」的 API Key，点击下方按钮一键配置。" }),
								configMsg ? (0, react_jsx_runtime.jsx)("div", { style: { fontSize: "12px", color: configMsg.indexOf("✅") === 0 ? "#30a46c" : "#e5484d", lineHeight: "1.5" }, children: configMsg }) : null,
								(0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", gap: "8px" }, children: [
									(0, react_jsx_runtime.jsx)("button", {
										type: "button",
										onClick: autoConfigureModel,
										disabled: configuring,
										style: {
											...BTN_PRIMARY,
											opacity: configuring ? 0.6 : 1,
											cursor: configuring ? "wait" : "pointer",
											fontSize: "12px", padding: "6px 14px", flex: 1
										},
										children: configuring ? "配置中…" : "🚀 一键配置"
									}),
									(0, react_jsx_runtime.jsx)("button", {
										type: "button",
										onClick: () => { setSniffedKey(null); setSniffedModel(null); setConfigMsg(""); },
										style: { ...BTN_OUTLINE, fontSize: "12px", padding: "6px 12px" },
										children: "忽略"
									})
								]})
							]})
