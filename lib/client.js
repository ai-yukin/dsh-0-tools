...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		let _healthProxyProbing = false;

		function probeHealthProxy() {
			if (_healthProxyPort) return Promise.resolve(_healthProxyPort);
			if (_healthProxyProbing) return Promise.resolve(null);
			_healthProxyProbing = true;
			return (async () => {
				...
				return null;
			})().finally(() => { _healthProxyProbing = false; });
		}
			...
			return new Promise((resolve) => {
				...
				dshApi("settings.describe", {}).then((desc) => {
					...
					return dshApi("credentials.describe", { refs: [block.apiKeyEnv] }).then((cred) => {
						...
						// v1.8.3：通过本地 Node 代理测速（绕过 CORS）
						return probeHealthProxy().then((proxyPort) => {
							if (!proxyPort) {
								// 代理不可用时回退：直接 fetch（会被 CORS 拦截，但至少有兜底）
								const start = Date.now();
								const ctrl = new AbortController();
								const timer = setTimeout(() => ctrl.abort(), HEALTH_TIMEOUT);
								const isOpenRouter = providerKey === "openrouter-free" || (block.baseURL && block.baseURL.indexOf("openrouter.ai") >= 0);
								const pingBody = { model: modelId, messages: [{ role: "user", content: "hi" }], max_tokens: 1, stream: false };
								if (isOpenRouter) pingBody.route = "free";
							...
							return fetch("http://127.0.0.1:" + proxyPort + "/ping", {
								method: "POST",
								headers: { "Content-Type": "application/json" },
								body: JSON.stringify({ baseURL: block.baseURL, apiKey: apiKey, modelId: modelId }),
					...
					return getCurrentDefaultModel().then((cur) => {
						...
						if (s && s.status === "unavailable" && (s.consecutiveUnavailable || 0) >= AUTO_SWITCH_THRESHOLD) {
							return switchToFastest().then((best) => {
								...
								// v1.8.3修复：切换失败时不静默忽略，记录状态便于排查
								console.warn("[dsh-0-tools] 自动切换失败:", e && e.message ? e.message : e);
							});
						}
					}).catch(() => { /* ignore */ });
				});
		...
		const GUIDE_MODELS = [
			...
			{ key: "qianfan", name: "百度千帆", desc: "ERNIE-Speed，QPS=50 高并发", color: "#2563eb", url: guideUrl("qianfan") },
