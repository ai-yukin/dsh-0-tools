...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 返回全量 provider 状态 dict { zai: bool, openrouter: bool, ... }，供
		...
		const DEFAULT_HELP = {
			...
			freeModels: [
				...
				{
					type: "openrouter",
					title: "OpenRouter Ox-Alpha 免费模型一键配置",
					badge: "New",
					desc: "Ox-Alpha 是 OpenRouter 上当前最热门的免费大模型（匿名预览版）：100 万字符超长上下文、单次最多输出 13.1 万字符、支持文本与图片输入，当前完全免费。",
					keysLabel: "OpenRouter 获取 Key（注册免绑卡）↗",
					keysUrl: OR_KEYS_URL,
					keyPlaceholder: "粘贴你的 OpenRouter API Key（sk-or- 开头）",
					keyPrefix: "sk-or-",
					endpoint: "/configure-openrouter",
					note: "匿名预览模型：官方无硬性截止日期，随时可能转收费、限流或下线；你的输入会被第三方留存，请勿提交敏感信息。"
				}
			]
		};
		...
		// ② 零费用·API配置中心：理念介绍 + 全部免费模型条目（由远程 freeModels 驱动，新增模型零发版）。
		function FreeConfigCenter(props) {
			const { items, status, refresh } = props;
			const list = items || [];
				...
				children: [
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsx)(FreeConfigCenter, { items: data.freeModels, status: status, refresh: refresh })
						]
					}),
					// ③ 零失控·费用管控中心
					(0, react_jsx_runtime.jsxs)("div", {
		...
		function FooterActions(props) {
			const { wide } = props;
			const { hasAnyInstalled } = useConfigured();
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					!hasAnyInstalled ? (0, react_jsx_runtime.jsx)(FreeApiButton, {}) : null,
