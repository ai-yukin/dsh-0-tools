	...
	factory: (require) => {
		...
		function updateHealthState(providerKey, latency) {
			...
			if (latency !== null && latency !== undefined) {
				...
				if (s.latencies.length === 0 || s.consecutiveUnavailable >= 2) s.status = "unavailable";
			}
		}
		...
		function ModelStatusIndicator(props) {
			...
			// v1.8.5: 从远程 help.json 获取模型简称（支持热更新新增模型）
			...
			// v1.8.5: 缩短文案，不显示"响应"二字
			const statusLabel = s.status === "ok" ? "正常" : (s.status === "slow" ? "较慢" : "超时");

			// v1.8.5: 找到最快可用的模型，用于"建议换XX"
			let fastestName = "";
			if (s.status === "unavailable" && health.state) {
				let fastestProvider = null;
				let fastestLatency = Infinity;
				for (const provider in health.state) {
					if (provider === currentProvider) continue;
					const state = health.state[provider];
					if (state && state.status === "ok" && state.avgLatency && state.avgLatency < fastestLatency) {
						fastestLatency = state.avgLatency;
						fastestProvider = provider;
					}
				}
				if (fastestProvider) {
					const fastestItem = freeModels.find((m) => m && m.type === fastestProvider);
					fastestName = (fastestItem && fastestItem.shortName) || fastestProvider;
				}
			}

			// v1.8.5: 只提示，不操作，按钮不可点击
			const displayText = s.status === "unavailable"
				? (fastestName ? currentModelName + "超时，建议换" + fastestName : currentModelName + "超时")
				: currentModelName + statusLabel;
			// v1.8.5: 状态文字（不含模型名），用于"模型名+三色球+状态"排列
			const statusTextAfter = s.status === "unavailable"
				? (fastestName ? "超时，建议换" + fastestName : "超时")
				: statusLabel;
			...
			return (0, react_jsx_runtime.jsx)("div", {
				...
				children: (0, react_jsx_runtime.jsxs)("button", {
					...
					children: [
						(0, react_jsx_runtime.jsxs)("span", { style: { display: "flex", alignItems: "center" }, children: [
		...
		function FooterActions(props) {
			const { wide, health, currentProvider, onOpenOnboarding, freeModelTypes } = props;
