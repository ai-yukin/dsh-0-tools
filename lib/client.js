...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 全局测速状态：{ providerKey: { latencies: [number], lastChecked: number, status: "ok"|"slow"|"unavailable", avgLatency: number } }
		const _healthState = {};
		...
		function updateHealthState(providerKey, latency) {
			if (!_healthState[providerKey]) {
				_healthState[providerKey] = { latencies: [], lastChecked: 0, status: "unknown", avgLatency: 0 };
			}
			...
			if (latency !== null && latency !== undefined) {
				...
				if (s.avgLatency < HEALTH_LATENCY_OK) s.status = "ok";
				else if (s.avgLatency < HEALTH_LATENCY_SLOW) s.status = "slow";
				else s.status = "unavailable";
			} else {
				// 测速失败：如果之前有历史，保留状态但标记 lastChecked；连续失败会被判定为不可用
				if (s.latencies.length === 0) s.status = "unavailable";
			}
		}
		...
		function runHealthCheck() {
			return readProviderStatus().then((status) => {
				const freeProviders = ["zai", "openrouter-free", "qianfan", "xinghuo"];
				const configured = freeProviders.filter((p) => status[p]);
				...
				children: (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						(0, react_jsx_runtime.jsx)("div", {
							...
							children: "选择一个免费模型，点击查看图文教程获取 API Key，复制后回到 DSH 粘贴即可一键配置。推荐至少接入 2 个，插件会自动帮你选最快的。"
						}),
						(0, react_jsx_runtime.jsx)("div", {
							style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" },
							children: GUIDE_MODELS.map((m) => (0, react_jsx_runtime.jsxs)("button", {
								type: "button",
								onClick: () => openExternal(m.url),
								style: {
									display: "flex", flexDirection: "column", alignItems: "flex-start", gap: "4px",
									padding: "14px", borderRadius: "12px", border: "1px solid #e0e0e8",
									background: "#fafbfc", cursor: "pointer", textAlign: "left",
									transition: "all 0.15s"
								},
								onMouseEnter: (e) => { e.currentTarget.style.borderColor = m.color; e.currentTarget.style.background = "#f0f5ff"; },
								onMouseLeave: (e) => { e.currentTarget.style.borderColor = "#e0e0e8"; e.currentTarget.style.background = "#fafbfc"; },
								children: [
									(0, react_jsx_runtime.jsx)("span", { style: { fontSize: "14px", fontWeight: 700, color: "#111" }, children: m.name }),
									(0, react_jsx_runtime.jsx)("span", { style: { fontSize: "11px", color: "#888", lineHeight: "1.4" }, children: m.desc }),
									(0, react_jsx_runtime.jsx)("span", { style: { fontSize: "11px", color: m.color, fontWeight: 600, marginTop: "2px" }, children: "查看教程 →" })
								]
							}, m.key))
						})
					]
		...
		function ModelStatusIndicator(props) {
			const { health, currentProvider, onSwitch } = props;
			const [switching, setSwitching] = _react.useState(false);
			const [msg, setMsg] = _react.useState("");

			const s = currentProvider ? health.state[currentProvider] : null;
			const isFreeModel = ["zai", "openrouter-free", "qianfan", "xinghuo"].indexOf(currentProvider) >= 0;
			...
			const latencyText = s.avgLatency > 0 ? s.avgLatency + "ms" : "测速中…";
					...
					(0, react_jsx_runtime.jsxs)("button", {
						...
						children: [
							(0, react_jsx_runtime.jsxs)("span", { style: { display: "flex", alignItems: "center", gap: "4px" }, children: [
								...
								(0, react_jsx_runtime.jsx)("span", { style: { fontWeight: 600 }, children: switching ? "切换中…" : (slowWarning ? "当前模型较慢，点此切最快" : "当前模型 · " + latencyText) })
							]}),
		...
		function FreeConfigCenter(props) {
			...
			const handleSwitchFastest = () => {
				if (switching || !health) return;
				setSwitching(true);
				setSwitchMsg("");
				health.switchFastest().then((result) => {
					setSwitching(false);
					setSwitchMsg("✅ 已切换到最快模型（" + result.avgLatency + "ms）");
					refresh();
					setTimeout(() => setSwitchMsg(""), 3000);
				}).catch((e) => {
					setSwitching(false);
					setSwitchMsg("⚠️ " + String((e && e.message) || e));
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					// v1.8.0: 健康监测仪表盘
					installedCount > 0 ? (0, react_jsx_runtime.jsxs)("div", {
						style: { background: "#f7f9fc", border: "1px solid #e3e8ef", borderRadius: "10px", padding: "12px" },
						children: [
							(0, react_jsx_runtime.jsxs)("div", {
								style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "10px" },
								children: [
									(0, react_jsx_runtime.jsx)("span", { style: { fontSize: "13px", fontWeight: 700, color: "#111" }, children: "📊 免费模型健康状态" }),
									(0, react_jsx_runtime.jsx)("button", {
										type: "button",
							...
							(0, react_jsx_runtime.jsx)("div", {
								...
								children: list.filter((m) => status[m.type]).map((m) => {
									const s = health && health.state[m.type];
									const isFastest = fastest && fastest.provider === m.type;
									const statusColor = !s ? "#999" : (s.status === "ok" ? "#30a46c" : (s.status === "slow" ? "#f5a623" : "#e5484d"));
									const statusText = !s ? "⏳" : (s.status === "ok" ? "🟢" : (s.status === "slow" ? "🟡" : "🔴"));
									const latencyText = !s || s.avgLatency === 0 ? "测速中" : s.avgLatency + "ms";
									return (0, react_jsx_runtime.jsxs)("div", {
										style: {
											display: "flex", alignItems: "center", justifyContent: "space-between",
											padding: "6px 8px", borderRadius: "6px",
											background: isFastest ? "#e8f5e9" : "#fff",
											border: "1px solid " + (isFastest ? "#a5d6a7" : "#e8e8e8")
										},
										children: [
											(0, react_jsx_runtime.jsxs)("span", { style: { display: "flex", alignItems: "center", gap: "4px", fontSize: "11px" }, children: [
												(0, react_jsx_runtime.jsx)("span", { children: statusText }),
												(0, react_jsx_runtime.jsx)("span", { style: { fontWeight: 600, color: "#333" }, children: MODEL_DISPLAY[m.type] || m.type }),
												isFastest ? (0, react_jsx_runtime.jsx)("span", { style: { fontSize: "9px", color: "#2e7d32", fontWeight: 700, background: "#c8e6c9", padding: "1px 4px", borderRadius: "4px" }, children: "最快" }) : null
											]}),
											(0, react_jsx_runtime.jsx)("span", { style: { fontSize: "10px", color: statusColor, fontWeight: 600 }, children: latencyText })
										]
									}, "health-" + m.type);
								})
							}),
							(0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									(0, react_jsx_runtime.jsx)("button", {
										type: "button",
										onClick: handleSwitchFastest,
										disabled: switching || !fastest,
										style: {
											...BTN_PRIMARY,
											opacity: switching || !fastest ? 0.6 : 1,
											cursor: switching || !fastest ? "wait" : "pointer",
											fontSize: "12px", padding: "5px 12px"
										},
										children: switching ? "切换中…" : "🚀 一键切到最快模型"
									}),
									(0, react_jsx_runtime.jsxs)("label", {
										...
										children: [
											...
											(0, react_jsx_runtime.jsx)("span", { children: "自动切换（当前模型变慢时自动换最快）" })
										]
									})
								]
							}),
							switchMsg ? (0, react_jsx_runtime.jsx)("div", { style: { fontSize: "11px", color: switchMsg.indexOf("✅") === 0 ? "#30a46c" : "#e5484d", marginTop: "6px" }, children: switchMsg }) : null
						]
					}) : null,
					(0, react_jsx_runtime.jsx)("div", {
						...
						children: [
							...
							" DeepSeek 官方 API 无永久免费模型，需充值才能调用；这里汇总当前可一键接入的免费模型，零费用先用起来，随时可一键卸载。接入多个后插件会自动测速，帮你选最快的。"
						]
					}),
					(0, react_jsx_runtime.jsx)("div", {
						...
						children: installedCount > 0
							...
							: "暂未接入免费模型，点击下方条目展开配置。推荐至少接入 2 个，自动帮你选最快的。"
					}),
		...
		function FooterActions(props) {
			const { wide, health, currentProvider, onOpenOnboarding, onSwitch } = props;
			...
			const isFreeModel = currentProvider && ["zai", "openrouter-free", "qianfan", "xinghuo"].indexOf(currentProvider) >= 0;
		...
		function FooterActionsWrapper(props) {
			...
			return (0, react_jsx_runtime.jsxs)(_react.Fragment, {
				children: [
					...
					showOnboarding ? (0, react_jsx_runtime.jsx)(OnboardingModal, { onClose: () => setShowOnboarding(false) }) : null
				]
			});
		}
